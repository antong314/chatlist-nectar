-- Canonical provider deletion/security setup for a fresh or existing project.
-- Keep COMMUNITY_DELETE_CODE only in Edge Function secrets. Rotate it with:
--   supabase secrets set COMMUNITY_DELETE_CODE=<new-code>
-- Rotating the code requires no database changes and affects new deletions only.

UPDATE public.contacts SET is_deleted = FALSE WHERE is_deleted IS NULL;
...
BEGIN
  FOR existing_policy IN
    ...
    WHERE policies.schemaname = 'public' AND policies.tablename = 'contacts'
  LOOP
...
GRANT INSERT (title, category, subtitle, phone_number, website_url, image_url, map_url)
  ON public.contacts TO anon, authenticated;
GRANT UPDATE (title, category, subtitle, phone_number, website_url, image_url, map_url)
  ON public.contacts TO anon, authenticated;
GRANT ALL ON TABLE public.contacts TO service_role;

CREATE POLICY "Public can read active contacts" ON public.contacts
  FOR SELECT TO anon, authenticated USING (is_deleted = FALSE);
CREATE POLICY "Public can add active contacts" ON public.contacts
  FOR INSERT TO anon, authenticated WITH CHECK (is_deleted = FALSE);
CREATE POLICY "Public can edit active contacts" ON public.contacts
  FOR UPDATE TO anon, authenticated USING (is_deleted = FALSE) WITH CHECK (is_deleted = FALSE);

DO $$
BEGIN
  CREATE TYPE public.provider_deletion_reason AS ENUM (
    'outdated', 'duplicate', 'closed', 'incorrect', 'other'
  );
EXCEPTION
...
CREATE INDEX IF NOT EXISTS provider_deletion_events_contact_deleted_idx
  ON public.provider_deletion_events (contact_id, deleted_at DESC);
CREATE INDEX IF NOT EXISTS provider_deletion_events_pending_undo_idx
  ON public.provider_deletion_events (undo_expires_at) WHERE undone_at IS NULL;
...
CREATE OR REPLACE FUNCTION public.perform_provider_soft_delete(
  p_contact_id UUID,
...
BEGIN
  SELECT contacts.title INTO v_provider_name
  FROM public.contacts AS contacts
  WHERE contacts.id = p_contact_id AND contacts.is_deleted = FALSE
  FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Provider not found or already removed' USING ERRCODE = 'P0002';
  END IF;

  v_confirmation := regexp_replace(lower(btrim(COALESCE(p_provider_name_confirmation, ''))), '[[:space:]]+', ' ', 'g');
  IF v_confirmation = '' OR v_confirmation <>
    regexp_replace(lower(btrim(COALESCE(v_provider_name, ''))), '[[:space:]]+', ' ', 'g')
  THEN
    RAISE EXCEPTION 'Provider name confirmation does not match' USING ERRCODE = '22023';
  END IF;

  v_reason := lower(btrim(COALESCE(p_reason, '')));
  IF v_reason NOT IN ('outdated', 'duplicate', 'closed', 'incorrect', 'other') THEN
    RAISE EXCEPTION 'Select a valid deletion reason' USING ERRCODE = '22023';
  END IF;

  v_requester_whatsapp := regexp_replace(btrim(COALESCE(p_requester_whatsapp, '')), '[[:space:]().-]+', '', 'g');
  ...
  IF v_requester_whatsapp !~ '^\+?[0-9]{8,15}$' THEN
    RAISE EXCEPTION 'Enter a valid WhatsApp number with 8 to 15 digits' USING ERRCODE = '22023';
  END IF;

  v_undo_token_hash := lower(btrim(COALESCE(p_undo_token_hash, '')));
  IF v_undo_token_hash !~ '^[0-9a-f]{64}$' THEN
    RAISE EXCEPTION 'Invalid undo token hash' USING ERRCODE = '22023';
  END IF;

  INSERT INTO public.provider_deletion_events (
    contact_id, provider_name_snapshot, reason, requester_whatsapp,
    undo_token_hash, deleted_at, undo_expires_at
  ) VALUES (
    p_contact_id,
  ...
  UPDATE public.contacts SET is_deleted = TRUE WHERE id = p_contact_id;
  RETURN QUERY SELECT v_event_id, v_undo_expires_at;
END;
$$;

CREATE OR REPLACE FUNCTION public.undo_provider_soft_delete(p_event_id UUID, p_undo_token_hash TEXT)
...
BEGIN
  ...
  IF NOT FOUND OR v_event.undone_at IS NOT NULL
    OR v_event.undo_token_hash <> v_undo_token_hash OR now() >= v_event.undo_expires_at
  THEN
    RAISE EXCEPTION 'Undo link is invalid or no longer available' USING ERRCODE = '22023';
  END IF;

  PERFORM 1 FROM public.contacts AS contacts WHERE contacts.id = v_event.contact_id FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Provider no longer exists' USING ERRCODE = 'P0002';
  END IF;
  UPDATE public.contacts SET is_deleted = FALSE WHERE id = v_event.contact_id;
  UPDATE public.provider_deletion_events SET undone_at = now() WHERE id = v_event.id;
END;
$$;
...
GRANT EXECUTE ON FUNCTION public.perform_provider_soft_delete(UUID, TEXT, TEXT, TEXT, TEXT)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.undo_provider_soft_delete(UUID, TEXT) TO service_role;
